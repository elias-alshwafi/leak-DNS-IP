...
class PreviewManager:
    ...
    def get_preview(self, system_id, bucket, lines=20, force_refresh=False):
        """
        Get preview with CORRECT API parameters for ALL buckets including redacted ones
        This is the critical fix from previous versions
        """
        # Check cache first
            ...
            with self.lock:
                ...
                if response.status_code == 200:
                    ...
                    if response.status_code == 400:
                        return "❌ Preview request failed: Invalid parameters"
                    elif response.status_code == 401:
                        return "❌ Preview request failed: Unauthorized"
                    elif response.status_code == 402:
                        return "❌ Preview request failed: No credits available"
                    elif response.status_code == 404:
                        return "❌ Preview request failed: Item not found"
                    else:
                        return f"❌ Preview request failed: HTTP {response.status_code}"
                    
        except Exception as e:
            return f"❌ Preview request error: {str(e)}"
    
    def _get_content_type(self, bucket):
        """Determine content type based on bucket with comprehensive mapping"""
...
class IntelligenceXCLI:
    ...
    def display_search_results(self):
        """Display search results in a clean, compact table with comprehensive bucket support"""
