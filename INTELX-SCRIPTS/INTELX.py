...
def _make_request(method, endpoint, json_data=None, params=None, stream=False):
    headers = {
        "x-key": INTELX_KEY,
        "User-Agent": "IntelX-CLI-Tool/4.1" # Updated User-Agent
    }
...
def post_search_options(records):
    while True:
        ...
        print("2. Download a specific file")
        ...
        if choice == 1:
            ...
            selected_record = records[index-1]
            system_id = selected_record.get("systemid")
            if system_id:
            ...
            if not records:
                ...
                continue
            index = get_user_input(f"Enter result number to download (1-{len(records)}): ", int, validation_func=lambda x: 1 <= x <= len(records), error_message="Invalid result number.")
            selected_record = records[index-1]
            system_id = selected_record.get("systemid")
            file_name = selected_record.get("name", f"downloaded_file_{system_id.split('-')[0]}") # Use name from record, or generate a default
            
            if system_id:
                # Sanitize filename for common OS compatibility
                sanitized_file_name = "".join([c for c in file_name if c.isalnum() or c in (".", "_", "-")]).strip()
                if not sanitized_file_name:
                    sanitized_file_name = f"downloaded_file_{system_id.split('-')[0]}"
                
                # Add a default extension if none exists and it's a common media type
                if "." not in sanitized_file_name:
                    media_type = selected_record.get("mediah", "").lower()
                    if "text file" in media_type:
                        sanitized_file_name += ".txt"
                    elif "csv file" in media_type:
                        sanitized_file_name += ".csv"
                    elif "pdf document" in media_type:
                        sanitized_file_name += ".pdf"
                    elif "word document" in media_type:
                        sanitized_file_name += ".docx"
                    elif "excel document" in media_type:
                        sanitized_file_name += ".xlsx"
                    elif "powerpoint document" in media_type:
                        sanitized_file_name += ".pptx"
                    elif "picture" in media_type:
                        sanitized_file_name += ".png" # Default to png, user can rename
                    elif "audio file" in media_type:
                        sanitized_file_name += ".mp3"
                    elif "video file" in media_type:
                        sanitized_file_name += ".mp4"
                    elif "html copy" in media_type or "html file" in media_type:
                        sanitized_file_name += ".html"
                    else:
                        sanitized_file_name += ".bin" # Generic binary

                read_file(system_id, sanitized_file_name)
            else:
                print("System ID not found for this record.")
        elif choice == 3:
            break
...
def view_file_detailed(system_id):
    ...
    if response:
        print("\n--- File Content (Full) ---")
        print(json.dumps(response, indent=2)) # Pretty print JSON response
        print("\n--------------------------")
